...
# ──────────────────────────────────────────────────────────────────────────────
# Description : Extract StarCraft II build orders from replay files.
#               Local development and automated VPS deployment.
...
help:
	...
	@printf "    $(STYLE_INSTRUCTION)make up$(RESET)          $(STYLE_DISCREET)•$(RESET) Build and start the container on http://localhost:$(LOCAL_PORT)\n"
	@printf "    $(STYLE_INSTRUCTION)make down$(RESET)        $(STYLE_DISCREET)•$(RESET) Stop the container\n"
	@printf "    $(STYLE_INSTRUCTION)make restart$(RESET)     $(STYLE_DISCREET)•$(RESET) Stop then start again\n"
	@printf "    $(STYLE_INSTRUCTION)make logs$(RESET)        $(STYLE_DISCREET)•$(RESET) Follow the local container logs\n"
	@printf "\n  $(BOLD)$(STYLE_SECTION)❯ Quality:$(RESET)\n"
	@printf "    $(STYLE_INSTRUCTION)make venv$(RESET)        $(STYLE_DISCREET)•$(RESET) Create the Python development environment\n"
	@printf "    $(STYLE_INSTRUCTION)make test$(RESET)        $(STYLE_DISCREET)•$(RESET) Run the test suite\n"
	@printf "\n  $(BOLD)$(STYLE_SECTION)❯ Command line:$(RESET)\n"
	@printf "    $(STYLE_INSTRUCTION)make extract$(RESET)     $(STYLE_DISCREET)•$(RESET) Extract the most recent replay (pass ARGS=...)\n"
	@printf "    $(STYLE_DISCREET)   e.g. make extract ARGS=\"--last 3 --cutoff 8:00 --clip\"$(RESET)\n"
	@printf "\n  $(BOLD)$(STYLE_SECTION)❯ Production (VPS $(VPS_SSH)):$(RESET)\n"
	@printf "    $(STYLE_INSTRUCTION)make deploy$(RESET)      $(STYLE_DISCREET)•$(RESET) Deploy to sc2.eole.me\n"
	@printf "    $(STYLE_INSTRUCTION)make checklogs$(RESET)   $(STYLE_DISCREET)•$(RESET) Follow the production logs\n"
	@printf "\n"
...
up:
	@printf "$(STYLE_TITLE)✨ Starting the local environment...$(RESET)\n"
	@if $(DOPPLER) --version >/dev/null 2>&1; then \
		printf "$(STYLE_PHASE)🔑 Syncing development secrets from Doppler...$(RESET)\n"; \
		if $(DOPPLER) secrets download --project $(DOPPLER_PROJECT) --config $(DOPPLER_CONFIG_DEV) --no-file --format env > .env.temp 2>/dev/null; then \
			...
			rm -f .env.temp; \
			printf "$(STYLE_RESULT)✅ Secrets synced.$(RESET)\n"; \
		else \
			rm -f .env.temp; \
			printf "$(STYLE_WARNING)⚠️  Doppler returned nothing, keeping the existing .env.$(RESET)\n"; \
			[ -f .env ] || cp $(DOCKER_DIR)/.env.example .env; \
		fi; \
	else \
		printf "$(STYLE_WARNING)⚠️  Doppler missing — using the local .env.$(RESET)\n"; \
		[ -f .env ] || cp $(DOCKER_DIR)/.env.example .env; \
	fi
	...
	if [ -n "$$NETWORK_NAME" ] && docker network inspect $$NETWORK_NAME >/dev/null 2>&1; then \
		printf "$(STYLE_RESULT)🔌 Shared network '$$NETWORK_NAME' detected, joining the eole.me stack.$(RESET)\n"; \
		DOCKER_NETWORK_EXTERNAL=true docker compose -f $(COMPOSE_DEV) --env-file .env up -d --build; \
	else \
		printf "$(STYLE_WARNING)ℹ️  No shared network: standalone mode.$(RESET)\n"; \
		DOCKER_NETWORK_NAME=sc2-build-order-standalone DOCKER_NETWORK_EXTERNAL=false \
			docker compose -f $(COMPOSE_DEV) up -d --build; \
	fi
	@printf "$(STYLE_RESULT)🚀 $(PROJECT_NAME) is ready on http://localhost:$(LOCAL_PORT)$(RESET)\n"

down:
	@printf "$(STYLE_WARNING)🛑 Stopping the local container...$(RESET)\n"
...
venv:
	@printf "$(STYLE_PHASE)🐍 Creating the Python environment...$(RESET)\n"
	@uv venv
	@uv pip install -r requirements-dev.txt
	@printf "$(STYLE_RESULT)✅ Ready. Run 'make test'.$(RESET)\n"

test:
	@printf "$(STYLE_PHASE)🧪 Tests...$(RESET)\n"
	@if [ -x .venv/bin/python ]; then .venv/bin/python -m pytest -q; \
	else printf "$(STYLE_WARNING)⚠️  No environment: run 'make venv'.$(RESET)\n"; exit 1; fi
...
_deploy:
	@printf "$(STYLE_PHASE)🚀 [1/4]$(RESET) Preparing the deployment space on $(BOLD)$(VPS_SSH)$(RESET)...\n"
	@ssh $(VPS_SSH) "mkdir -p $(VPS_PATH)" >/dev/null
	@printf "$(STYLE_PHASE)📦 [2/4]$(RESET) Uploading the configuration...\n"
	...
	@if $(DOPPLER) --version >/dev/null 2>&1; then \
		if $(DOPPLER) secrets download --project $(DOPPLER_PROJECT) --config $(DOPPLER_CONFIG_PROD) --no-file --format env > .env.prod.temp 2>/dev/null; then \
			...
			printf "$(STYLE_ERROR)❌ Doppler secrets download failed.$(RESET)\n"; \
			rm -f .env.prod.temp; exit 1; \
		fi; \
	else \
		printf "$(STYLE_ERROR)❌ Doppler not found in PATH.$(RESET)\n"; exit 1; \
	fi
	@printf "$(STYLE_PHASE)🐳 [4/4]$(RESET) Pulling the image and restarting...\n"
	...
	@printf "$(STYLE_RESULT)✅ $(PROJECT_NAME) [$(VERSION) / $(VPS_PROJECT_TAG)] deployed to https://sc2.eole.me$(RESET)\n"

checklogs:
	@printf "$(STYLE_PHASE)📟 Production logs [$(VPS_SSH)]...$(RESET)\n"
